...
switch($method) {
    case 'GET':
        $stmt = $conn->prepare("SELECT * FROM immunization_notes");
        $stmt->execute();
    ...
    case 'POST':
        ...
        $stmt = $conn->prepare("INSERT INTO immunization_notes (title, content) VALUES (:title, :content)");
    ...
    case 'PUT':
        ...
        $stmt = $conn->prepare("UPDATE immunization_notes SET title = :title, content = :content WHERE id = :id");
        ...
        case 'DELETE':
            ...
            $id = intval($_GET['id']);
            $stmt = $conn->prepare("DELETE FROM immunization_notes WHERE id = :id");
