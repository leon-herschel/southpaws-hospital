...
const ReportGeneration = () => {
    ...
    const fetchData = () => {
        ...
        axios.get(`http://localhost:80/api/reports.php?action=${filter}`, { params })
            .then(response => {
                console.log("📥 Fetched Data:", response.data);

                if (response.data.status === 1 && Array.isArray(response.data.data)) {
                    setData(response.data.data);
                    setColumns(response.data.data.length > 0 ? Object.keys(response.data.data[0]) : []);
                } else {
...
const formatDate = (dateStr) => {
    ...
    const getSortIcon = (key) => sortBy.key === key ? (sortBy.order === 'asc' ? <AiOutlineArrowUp /> : <AiOutlineArrowDown />) : null;
    ...
    const handlePrint = () => {
        ...
        const reportTitle = `${filter.charAt(0).toUpperCase() + filter.slice(1)} Report`;
        ...
        <div className='container mt-2'>
            ...
            <Nav variant="tabs" defaultActiveKey="sales">
                {['sales', 'products', 'services', 'clients'].map(type => (
                    <Nav.Item key={type}>
                        <Nav.Link eventKey={type} onClick={() => setFilter(type)}>{type.charAt(0).toUpperCase() + type.slice(1)}</Nav.Link>
                    </Nav.Item>
                ))}
            </Nav>
            <input 
    ...
    {currentData.map((item, key) => (
        <tr key={key}>
            {columns.map((col, index) => {
                ...
                if (col.toLowerCase().includes('date') || col.toLowerCase().includes('created_at') || col.toLowerCase().includes('updated_at')) {
                    value = formatDate(value);
                }

                // ✅ Convert string numbers to float for proper formatting
                if (value && (col.toLowerCase().includes('price') || col.toLowerCase().includes('total') || col.toLowerCase().includes('amount'))) {
                    value = parseFloat(value); // Ensure it's a number
                    value = !isNaN(value) ? `₱${value.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : 'N/A';
                }

                return <td key={index} className="text-center">{value || 'N/A'}</td>;
            })}
        </tr>
    ))}
</tbody>
