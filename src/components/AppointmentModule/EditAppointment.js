...
const EditAppointment = ({ show, onClose, eventData, onUpdated }) => {
  ...
  const handleUpdate = async () => {
    ...
    const hasNewEmail =
      ...
      trimmedEmail.toLowerCase() !== "no_email@noemail.com";
    ...
    const wasCancelled = eventData.status === "Cancelled";
    const isNowConfirmed = formData.status === "Confirmed";

    // Check if key appointment info changed
    const infoChanged =
      formData.pet_name !== eventData.pet_name ||
      formData.date !== eventData.startDate ||
      formData.time !== eventData.startTime ||
      formData.end_time !== eventData.endTime ||
      formData.service.join(", ") !== eventData.service ||
      String(formData.doctor_id) !== String(eventData.doctor_id);

    if (formData.status === "Cancelled" || formData.status === "Done") {
      await updateAppointment(false);
    } else if (
        (hasNewEmail ||
          (wasCancelled && isNowConfirmed) ||
          (isNowConfirmed && infoChanged)) &&
        hasValidEmail && 
        trimmedEmail.toLowerCase() !== "no_email@noemail.com"
      ) {
      let toastClosedByChoice = false;

      toast.info(
        <div>
          <p>
            {hasNewEmail
              ? "New email detected. Send confirmation email?"
              : wasCancelled && isNowConfirmed
              ? "This appointment was previously cancelled. Send a new confirmation email?"
              : "Important appointment details changed. Send updated confirmation email?"}
          </p>
          <div className="d-flex gap-2">
            <button
              className="btn btn-success btn-sm"
              onClick={() => {
                toastClosedByChoice = true;
                toast.dismiss();
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const updated = isChecked
                          ...
                          : formData.service.filter((s) => s !== service.name);
