...
const History = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:80/api/history_api.php`, { params });
        ...
        <div className='container mt-4'>
            ...
            <div className="d-flex justify-content-between align-items-center">
                <div className="d-flex">
                    ...
                    <input type="date" id="fromDate" className="form-control me-3"
        ...
        <tr key={key}>
            {columns.map((col, index) => (
                <td key={index} className="text-center">
                    {col === 'created_at' || col === 'updated_at' ? formatDate(item[col]) :
                     col === 'created_by' ? (item['created_by'] ?? 'N/A') :
                     col === 'confirmed_by' ? (item['confirmed_by'] ?? 'N/A') :
                     item[col] ?? 'N/A'}
                </td>
            ))}
        </tr>
