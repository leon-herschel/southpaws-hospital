...
const PointofSales = () => {
  ...
  const [isImmunizationFormComplete, setIsImmunizationFormComplete] =
    useState(false);
  ...
  const handleReferenceSelect = async (clientId) => {
    try {
      ...
      if (!selectedPetName) {
        console.warn(
          "No pet name matched, skipping appointment service fetch."
        );
        return;
      }
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "Enter") {
      ...
      // Find a client with an exact name match (case-insensitive)
      ...
      if (exactMatch) {
        // Manually trigger the client change handler with the matched client's ID
        handleClientChange({ target: { value: exactMatch.id } });
      }
    }
  };
  ...
  const fetchPets = (clientId) => {
    if (clientId) {
      axios
        .get(`http://localhost:80/api/clients.php/${clientId}`)
        .then((response) => {
  ...
  const handleServiceForm = (service) => {
    ...
    const selectedPetsData = clientPets.filter((pet) =>
      selectedPets.includes(pet.pet_id)
    );
  ...
  const fetchClients = () => {
    axios
      .get("http://localhost:80/api/clients.php/")
      .then((response) => {
  ...
  const fetchInventory = () => {
    axios
      ...
      .then((response) => {
        const availableInventory = (response.data.inventory || []).filter(
  ...
  const fetchServices = () => {
    axios
      .get("http://localhost:80/api/services.php")
      .then((response) => {
        let availableServices = response.data.filter(
  ...
  const addProductToCart = (item) => {
    ...
    if (existingItem) {
      ...
      setCartItems([
        ...cartItems,
        { ...item, quantity: 1, product_id: item.product_id },
      ]);
    }
  ...
  const addServiceToCart = (service) => {
    ...
    if (!existingService) {
      setCartItems([
        ...cartItems,
        { ...service, isService: true, quantity: 1, selectedPets },
      ]);
  ...
  const handleConfirm = () => {
    ...
    const incompleteForms = cartItems.some((item) => {
      if (
        item.consent_form === "Immunization Form" &&
        !isImmunizationFormComplete
      ) {
    ...
    if (incompleteForms) {
      setErrorMessage(
        "Please complete all required forms before confirming payment."
      );
      return;
    }
  ...
  const updateCartItemQuantity = (item, newQuantity) => {
    ...
    setCartItems((prevCart) =>
      prevCart.map((cartItem) =>
        cartItem.barcode === item.barcode ? { ...cartItem, quantity } : cartItem
      )
    );
  };
  ...
  const handleConfirmYes = async () => {
    try {
      ...
      const taxRate =
        taxResponse.data.status === 1 ? parseFloat(taxResponse.data.tax) : 0; // Default to 0 if not set
      ...
      if (!clientId) {
        ...
        try {
          ...
          if (clientResponse.data.status === 1) {
            unregisteredClientIdToUse =
              clientResponse.data.unregistered_client_id; // Corrected variable
      ...
      const response = await axios.post(
        "http://localhost:80/api/orders.php?action=create_order",
        receiptData
      );

      if (response.data.status === 1) {
        const generatedReceiptNumber =
          response.data.receipt_number || "Unknown";
        ...
        if (selectedPetName && clientId) {
          try {
            await axios.post(
              "http://localhost/api/POS-Integration/updateAppointmentStatus.php",
              {
                client_id: clientId,
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        backgroundColor: selectedPets.includes(pet.pet_id)
                          ? "#007BFF"
                          : "#fff",
                        color: selectedPets.includes(pet.pet_id)
                          ? "white"
                          : "transparent",
                        textAlign: "center",
                        fontSize: "14px",
      ...
      <div className="d-flex justify-content-around mt-4">
        ...
        >
          <Card.Body>
            ...
            <Card.Text className="text-center">
              Click to view and add items to the cart.
            </Card.Text>
          </Card.Body>
        </Card>
        <Card
        ...
        >
          <Card.Body>
            ...
            <Card.Text className="text-center">
              Click to view and add services to the cart.
            </Card.Text>
          </Card.Body>
        </Card>
      </div>
