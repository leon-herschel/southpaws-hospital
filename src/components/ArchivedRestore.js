...
const ArchivedRestore = () => {
    ...
    const fetchData = () => {
        axios.get(`http://localhost:80/api/restore.php?action=all`)
            .then((response) => {
    ...
    const confirmAction = () => {
        ...
        axios.post(`http://localhost:80/api/${apiEndpoint}`, {
    ...
    return (
        <div className="container mt-4">
            ...
            <div className="table-responsive mt-3">
                <table className="table table-striped table-hover custom-table">
                    ...
                    <tbody>
                        {data.length === 0 ? (
                            <tr>
                                <td colSpan={3} className="text-center text-muted">No archived records found</td>
                            </tr>
                        ) : (
                            Object.entries(
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                ...
                <Modal.Body>Are you sure you want to {modalAction} the selected records?</Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={() => setShowModal(false)}>Cancel</Button>
                    <Button variant={modalAction === "delete" ? "danger" : "success"} onClick={confirmAction}>Confirm</Button>
                </Modal.Footer>
            </Modal>
        </div>
    );
