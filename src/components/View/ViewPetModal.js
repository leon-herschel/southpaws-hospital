...
const ViewPetModal = ({ show, handleClose, pet }) => {
  ...
  const [showAddMedicalRecordModal, setShowAddMedicalRecordModal] =
    useState(false); // For adding medical history
  const [showViewMedicalRecordModal, setShowViewMedicalRecordModal] =
    useState(false); // For viewing medical history
  ...
  const fetchAvailedServices = async (petId) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/orders.php?pet_id=${petId}`
      );
  ...
  const fetchMedicalHistory = async (petId) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/medical_records.php?pet_id=${petId}`
      );
        ...
        {loading ? (
          ...
          <Form>
                          ...
                          <tr key={index}>
                            ...
                            <td>
                              <Button
                                variant="info"
                                onClick={() =>
                                  handleOpenViewMedicalRecordModal(history)
                                }
                                size="sm"
                              >
                                <FaEye /> View
            ...
            <div className="button-container text-center mt-4">
              <Button
                variant="primary"
                onClick={handleOpenAddMedicalRecordModal}
