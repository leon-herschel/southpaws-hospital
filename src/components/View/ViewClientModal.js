...
const ViewClientModal = ({ show, handleClose, client = {} }) => {
  ...
  const fetchPets = () => {
    if (client?.id) {
      axios
        .get(`${API_BASE_URL}/api/clients.php/${client.id}`)
        .then((response) => {
  ...
  const deletePet = (petId) => {
    axios
      .delete(`${API_BASE_URL}/api/patients.php/${petId}/delete`)
      .then((response) => {
  ...
  const handleEditSubmit = (updatedPet) => {
    axios
      .put(
        `${API_BASE_URL}/api/patients.php/${updatedPet.pet_id}/update`,
        updatedPet
      )
      .then((response) => {
      ...
      <Modal show={show} onHide={handleClose} centered size="lg">
        ...
        <Modal.Body>
          <Row>
            ...
            <Col md={6}>
              <p>
                <strong>Cell Number:</strong> {client?.cellnumber || "N/A"}
              </p>
              <p>
